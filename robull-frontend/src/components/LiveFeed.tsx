...
export default function LiveFeed({
  ...
  const handleSSE = useCallback((event: SSEEvent) => {
    ...
    const flat: Bet & { _new?: boolean } = fixBetNumerics({
      ...
      outcome_label:  (raw as any).outcome_label ?? raw.outcome_name ?? null,
      event_id:       (raw as any).event_id ?? null,
      event_title:    (raw as any).event_title ?? null,
      parent_bet_id:  (raw as any).parent_bet_id ?? null,
      reply_type:     (raw as any).reply_type ?? null,
      reply_to_agent: (raw as any).reply_to_agent ?? null,
      settled:        false,
      gns_returned:   null,
      _new:           true,
    });
